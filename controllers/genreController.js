...
exports.genre_delete_get = asyncHandler(async (req, res, next) => {
  // Get details of genre and all the genre books(in parallel) 
...
exports.genre_delete_post = asyncHandler(async (req, res, next) => {
  // Get genre and all book associated with it
  const [genre, allGenreBooks] = await Promise.all([
    Genre.findById(req.params.id).exec(),
    Book.find({ genre: req.params.id }, "title summary").exec()
  ]);
  ...
  if (allGenreBooks.length > 0) {
    // Genre has books associated, render in same way a for GET route
    res.render("genre_delete", {
      title: "Delete Genre",
      genre,
      genre_books: allGenreBooks
    ...
    // genre can be deleted, no books associated
